const filterAction = require('../pageobjects/filter/filterAction');
...
describe("Swag Labs product purchase journey two", () => {
    ...
    it("successfully verifies filter", async () => {
      await filterAction.clickFilterDropdown();
      await filterAction.clickSpecificDropdown();
    });
    it("should select first products and add to cart", async () => {
        const products = await addToCartLocator.productList;
        const numberOfProductsToSelect = 1;
